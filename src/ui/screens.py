...
class PreLoadWindow(Screen):   
    StatusMessages = ["Blueing...", "Frening...", "VPNing...", "Done"]
    title = "BlueVPN"
    k = 0
    j = 0
...
class MainWindow(Screen):
    ...
    NewWallet = False
    box_color = ColorProperty('#179cdf')
    clock = None
    PersistentBandwidth = {}
    ...
    def build_meile_map(self):
        if not self.MeileMapBuilt: 
            ...
            source = MapSource(url="https://server.arcgisonline.com/arcgis/rest/services/Canvas/World_Light_Gray_Base/MapServer/tile/{z}/{y}/{x}.png",
                               cache_key="meile-map-canvas-light-grey-base", 
                               tile_size=512,
                               image_ext="png",
                               attribution="@ Meile",
                               size_hint=(.7,1))
